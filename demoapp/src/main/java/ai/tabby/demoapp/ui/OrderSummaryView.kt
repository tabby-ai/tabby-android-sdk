...
/** Small order summary shown alongside the checkout flow so the customer sees what they're paying for. */
@Composable
fun OrderSummaryView(tabbyPayment: TabbyPayment) {
    Column(
        verticalArrangement = Arrangement.Center,
        horizontalAlignment = Alignment.Start
    ) {
        Text(text = "Order:", fontStyle = FontStyle.Italic, fontSize = 20.sp)
        Spacer(modifier = Modifier.height(18.dp))
        OrderItemsWidget(order = tabbyPayment.order)
        Spacer(modifier = Modifier.height(18.dp))
        Text(
...
@Composable
fun OrderItemsWidget(order: Order?) {
